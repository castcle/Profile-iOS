...
public final class SyncSocialMediaViewModel {
        ...
        self.userRepository.getUser(userId: self.castcleId) { (success, response, isRefreshToken) in
            if success {
                do {
                    ...
                    self.checkIsSyncTwitter()
                    self.checkIsSyncFacebook()
                    self.checkIsDuplicate()
                } catch {
                    self.didError?()
                }
            } else {
    ...
    private func checkIsSyncTwitter() {
    ...
    private func checkIsSyncFacebook() {
